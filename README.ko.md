...
helper의 역할은 그대로입니다 — 돌기 전에 릴리스를 증명하는 것. 그리고 지금 고정하고 있는 릴리스인 TCRN Workflow `v0.1.0-rc.6`은, 스킬의 레퍼런스가 운영자에게 다루는 법을 가르치는 통제된 표면을 제공합니다.
...
- `0.1.0-candidate.9`는 **프리릴리스 후보**이며 정확히 TCRN Workflow `v0.1.0-rc.6`을 지원합니다.
