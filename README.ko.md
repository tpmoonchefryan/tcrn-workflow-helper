...
![status](https://img.shields.io/badge/status-0.1.0--candidate.37-blue) ![deps](https://img.shields.io/badge/dependencies-0-success) ![files](https://img.shields.io/badge/bootstrap-1%20file-brightgreen) ![force](https://img.shields.io/badge/--force-does%20not%20exist-critical)

![license](https://img.shields.io/badge/license-Apache--2.0-lightgrey) ![node](https://img.shields.io/badge/node-%E2%89%A5%2024-informational) ![network](https://img.shields.io/badge/network-none-important) ![hosts](https://img.shields.io/badge/hosts-Claude%20Code%20%C2%B7%20Codex-blueviolet) ![supports](https://img.shields.io/badge/TCRN%20Workflow-v0.11.12-blue)
...
# bc59f6e5b533e890863a0d2339922aae2240ccd7e5b67a90e99a3d770a9a09c4
```
...
helper의 역할은 그대로입니다 — 돌기 전에 릴리스를 증명하는 것. 그리고 지금 고정하고 있는 릴리스인 TCRN Workflow `v0.11.12`은, 스킬의 레퍼런스가 운영자에게 다루는 법을 가르치는 통제된 표면을 제공합니다.
...
- **통제된 재배치** — 워크스페이스는 새 경로나 새 기계로 가는, 기록에 남는 경로를 갖습니다(`relocation-plan` / `-vacate` / `-adopt` / `-abort` / `-inspect`). 이 동사들이 옮기는 것은 바인딩이고, 바이트는 운영자가 옮기며, 어떤 이벤트도 다시 쓰이지 않습니다. 분기를 막지는 못하고 분기를 보이게 만듭니다 — 의존하기 전에 릴리스의 `docs/adr/0003-workspace-relocation.md`를 읽으십시오.
- **읽을 수 있는 체인** — `event-list`는 이벤트를 그대로, 페이지 단위로 반환하므로 `export`가 거부할 만큼 큰 체인도 소비자가 다시 도출할 수 있습니다.
...
- `0.1.0-candidate.37`는 **프리릴리스 후보**이며 정확히 TCRN Workflow `v0.11.12`을 지원합니다.
