...
`ステータス: 0.1.0-candidate.2(プレリリース候補)` · `ライセンス: Apache-2.0` · `Node ≥ 24` · `依存関係: ゼロ` · `対応: TCRN Workflow v0.1.0-rc.4`
...
### なぜ本物の Codex / Claude Code スキルロケーションにインストールできないのか?

この候補の任務は検証であり、アクティベーションではないからです。ライブホストへのインストールは別途ゲートされたリリース判断です。それまでガードは構造的です:字句的ライブロケーション検査はテストルートマーカー検査より前、あらゆるファイルシステムプローブより前に走り、大文字小文字を畳み込み(そのため大文字小文字を区別しないファイルシステムの `.Claude` もすり抜けられません)、両ホストのユーザーレベル・プロジェクトレベル・大文字小文字バリアント形状のテストで覆われています。
...
**70 テスト、すべてオフライン**(唯一の `node:net` 使用は特殊ファイル拒否テスト用のローカル unix ドメインソケットフィクスチャ):
...
- `0.1.0-candidate.2` は TCRN Workflow `v0.1.0-rc.4` をちょうど対象とする**プレリリース候補**です。
