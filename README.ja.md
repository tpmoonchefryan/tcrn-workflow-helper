...
helper の役割は変わりません——動く前にリリースを証明すること。そして固定しているリリース、TCRN Workflow `v0.14.0` は、スキルのリファレンスがオペレーターに操作を教える統制された面を備えています。
...
- `0.1.0-candidate.47` は**プレリリース候補**で、TCRN Workflow `v0.14.0` をちょうど対象とします。
