...
`Status: 0.1.0-candidate.2 (pre-release candidate)` · `License: Apache-2.0` · `Node ≥ 24` · `Dependencies: zero` · `Supports: TCRN Workflow v0.1.0-rc.4`
...
### Why can't it install into a real Codex or Claude Code skill location?

Because this candidate's mandate is validation, not activation. Live-host installation is a separately gated release decision. Until then, the guard is structural: the lexical live-location check runs before the test-root marker check and before any filesystem probe, is case-folded (so `.Claude` on a case-insensitive filesystem cannot slip past), and is covered by tests for user-level, project-level, and case-variant shapes on both hosts.
...
**70 tests, all offline** (the only `node:net` use is a local unix-domain-socket fixture for special-file rejection):
...
| `test/` | The 70-test proof suite. |

## Status, honestly

- `0.1.0-candidate.2` is a **pre-release candidate** supporting exactly TCRN Workflow `v0.1.0-rc.4`.
