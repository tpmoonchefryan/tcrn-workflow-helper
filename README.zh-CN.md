...
**77 个测试，全部离线**（唯一用到 `node:net` 的地方，是一个用于特殊文件拒绝的本地 unix 域套接字 fixture）：
...
Helper 的职责没有变——在它运行之前证明这份发布。而它所钉定的这个版本，TCRN Workflow `v0.1.0-rc.6`，带来了一个受治理的表面，技能的参考文档会教操作者去驱动它：
...
- `0.1.0-candidate.9` 是一个**预发布候选**，恰好支持 TCRN Workflow `v0.1.0-rc.6`。
