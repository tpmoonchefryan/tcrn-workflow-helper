...
fragments generated by the current Workflow `host-render` projection; the
historical adapter family is not a current CLI surface. The **user-owned zone**
may contain arbitrary hooks and must be preserved byte-for-byte; user hooks are
not reported as TCRN baseline findings. A host-render update changes only its
managed fragment. A stop-pact hook that is not on the managed baseline is an
explicitly recorded independent/Owner-boundary item, not an excuse to write the
user zone.
